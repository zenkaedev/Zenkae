...
function toStyle(v: IconVariant): ButtonStyle {
  switch (v) {
    case 'ok': return ButtonStyle.Success;      // verde (afirma)
    case 'danger': return ButtonStyle.Danger;   // vermelho (nega)
    case 'primary': return ButtonStyle.Primary; // azul Discord (usar pouco)
    default: return ButtonStyle.Secondary;      // cinza (padrão)
  }
}

// Botão só-ícone (sem label)
export function iconButton(customId: string, emojiPath: EmojiPath, variant: IconVariant = 'neutral') {
  const e = getEmoji(emojiPath);
  return new ButtonBuilder().setCustomId(customId).setStyle(toStyle(variant)).setEmoji({ id: e.id, name: e.name });
}
...
export function navRow(
  ...
  if (opts.back ?? true)    items.push(iconButton(`${baseId}:back`, 'navigation.arrow_back', 'neutral'));
  if (opts.refresh ?? true) items.push(iconButton(`${baseId}:refresh`, 'navigation.arrow_refresh', 'neutral'));
  if (opts.next ?? true)    items.push(iconButton(`${baseId}:next`, 'navigation.arrow_next', 'neutral'));
  if (opts.upDown) {
