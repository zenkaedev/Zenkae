...
import { recruitDrafts } from '../../modules/recruit/store.drafts';
import { buildApplicationCard } from '../../modules/recruit/card';
...
export async function renderPublicRecruitPanelV2(guildId: string) {
  ...
  // Top image (opcional)
  const topUrl = (s.appearanceImageUrl || '').trim();
  if (topUrl) {
    containerChildren.push({
      type: V2.MediaGallery,
      items: [{ media: { url: topUrl } }],
    });
  }
  ...
  const desc = (s.appearanceDescription || '').trim();
  if (desc) {
    headerTexts.push({ type: V2.TextDisplay, content: desc });
  }

  // Section com accessory SOMENTE se houver URL válida
  const section: any = {
    type: V2.Section,
    components: headerTexts,
  };
  const thumb = String((s as any).appearanceThumbUrl ?? '').trim();
  if (thumb) {
    section.accessory = { type: V2.Thumbnail, media: { url: thumb } };
  }
  ...
  // Classes → opções
  const classes = ((s as any).classes ?? []) as Array<{
    id: string; name: string; emoji?: string | null; roleId?: string | null
  }>;
  const options =
...
export async function handleStartClick(inter: ButtonInteraction) {
  ...
  const classes = ((s as any).classes ?? []) as Array<{ id: string; name: string }>;
  const cls = classes.find((c) => String(c.id) === String(draft.classId));
  const className = cls?.name ?? '—';

  const activityCount =
    (await (recruitStore as any).getMessageCount?.(guildId, userId).catch(() => 0)) ?? 0;
...
async function publishApplication(
  ...
  if (!app) {
    await (interaction as any)
      .reply?.({ ephemeral: true, content: '❌ Aplicação não encontrada.' })
      .catch(() => null);
    return;
  }
