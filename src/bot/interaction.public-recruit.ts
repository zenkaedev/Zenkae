...
export async function routePublicRecruit(i: any) {
  try {
    if (
      ...
      i.commandName === 'recruit' &&
      i.options?.getSubcommand?.(false) === 'publishV2'
    ) {
      await publishPublicRecruitPanelV2(i);
      return true;
    }
    if (i.isStringSelectMenu?.() && i.customId === PUB_IDS.classSelect)
      return await handleClassSelect(i);
    if (i.isButton?.()) {
    ...
    if (i.isModalSubmit?.()) {
      ...
      if (i.customId.startsWith(PUB_IDS.applyQModalPrefix))
        return await handleApplyQuestionsSubmit(i);
    }
  } catch (e) {
