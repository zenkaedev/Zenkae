  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    try {
      ...
      if (interaction.isButton() && interaction.customId.startsWith('recruit:apply:q:open:')) {
        await openApplyQuestionsModal(interaction, interaction.customId.split(':').pop()!);
        return;
      }
      if (
        interaction.isModalSubmit() &&
        interaction.customId.startsWith('recruit:apply:q:modal:')
      ) {
        await ensureDeferredModal(interaction);
        await handleApplyQuestionsSubmit(interaction, interaction.customId.split(':').pop()!);
        return;
      }
      ...
      if (
        ...
        interaction.customId === ids.recruit.selectPanelChannel
      ) {
        if (!(await assertStaff(interaction))) return;
      ...
      if (
        interaction.isChannelSelectMenu() &&
        interaction.customId === ids.recruit.selectFormsChannel
      ) {
        if (!(await assertStaff(interaction))) return;
      ...
      if (
        ...
        interaction.customId === ids.recruit.settingsClasses
      ) {
        if (!(await assertStaff(interaction))) return;
      ...
      if (interaction.isButton() && ids.recruit.isClassEdit(interaction.customId)) {
        if (!(await assertStaff(interaction))) return;
        await openClassModal(interaction, interaction.customId.split(':').pop()!);
        return;
      }
      if (interaction.isButton() && ids.recruit.isClassRemove(interaction.customId)) {
        if (!(await assertStaff(interaction))) return;
        await handleClassRemove(interaction, interaction.customId.split(':').pop()!);
        return;
      }
      ...
      if (
        interaction.isModalSubmit() &&
        (interaction.customId === ids.recruit.modalClassSave ||
          ids.recruit.isModalClassUpdate(interaction.customId))
      ) {
        if (!(await assertStaff(interaction))) return;
        await ensureDeferredModal(interaction);
      ...
      if (interaction.isButton() && ids.recruit.isApprove(interaction.customId)) {
        if (!(await assertStaff(interaction))) return;
        await handleDecisionApprove(interaction, interaction.customId.split(':').pop()!);
        return;
      }
      if (interaction.isButton() && ids.recruit.isReject(interaction.customId)) {
        if (!(await assertStaff(interaction))) return;
        await handleDecisionRejectOpen(interaction, interaction.customId.split(':').pop()!);
        return;
      }
      if (
        interaction.isModalSubmit() &&
        interaction.customId.startsWith('recruit:decision:reject:modal:')
      ) {
        if (!(await assertStaff(interaction))) return;
        await ensureDeferredModal(interaction);
        await handleDecisionRejectSubmit(interaction, interaction.customId.split(':').pop()!);
        return;
      }
      ...
      if (
        interaction.isModalSubmit() &&
        interaction.customId.startsWith(PUB_IDS.applyQModalPrefix)
      ) {
        await ensureDeferredModal(interaction);
